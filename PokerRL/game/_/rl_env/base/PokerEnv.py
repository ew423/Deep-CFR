...
class PokerEnv:
    ...
    # Divisor for chip numbers to compute e.g. MBB/H from raw chip numbers in eval.
    EV_NORMALIZER = NotImplementedError
    # To plot graphs in relation to a fixed quantity (e.g. Poker.MeasureAnte)
    WIN_METRIC = NotImplementedError
    ...
    # number of possible unique combinations of holecards
    RANGE_SIZE = NotImplementedError
    ...
    # obs modes
    # Whether suits matter in the ruleset played. (i.e. do flushes exist?)
    SUITS_MATTER = NotImplementedError
    ...
    def __init__(self,
        ...
        # PokerPlayer instance of player currently having to get_action
    ...
    def _construct_obs_space(self):
        ...
        if (self.N_SEATS == 2) and self._USE_SIMPLE_HU_OBS:
            ...
            _table_space = [  # (blinds are in obs to give the agent a perspective on starting stack after normalization
                # .................................... self.ANTE
                get_new_box("ante", next_idx, self.N_SEATS),
                # ............................. self.SMALL_BLIND
                get_new_box("small_blind", next_idx, self.N_SEATS),
                # ............................... self.BIG_BLIND
                get_new_box("big_blind", next_idx, self.N_SEATS),
                # ............................... min total raise
                get_new_box("min_raise", next_idx, self.N_SEATS),
                # ................................. main_pot amount
                get_new_box("pot_amt", next_idx, self.N_SEATS),
                # ........................... total_to_call
                get_new_box("total_to_call", next_idx, self.N_SEATS),
                # .................... self.last_action[1]
                get_new_box("last_action_how_much", next_idx, self.N_SEATS),
            ]
            # .................................................................. self.last_action[0]
            for i in range(3):
                _table_space.append(get_discrete(
                    1, "last_action_what_" + str(i), next_idx))

            for i in range(
                    self.N_SEATS):  # ....................................................... self.last_action[2]
                _table_space.append(get_discrete(
                    1, "last_action_who_" + str(i), next_idx))

            for i in range(
                    self.N_SEATS):  # ....................................................... curr_player.seat_id
                _table_space.append(get_discrete(
                    1, "p" + str(i) + "_acts_next", next_idx))

            # ...................................... round onehot
            for i in range(max(self.ALL_ROUNDS_LIST) + 1):
                _table_space.append(get_discrete(
                    1, "round_" + Poker.INT2STRING_ROUND[i], next_idx)),
            ...
            for i in range(self.N_SEATS):
                ...
                _player_space += [
                    # ..................... stack
                    get_new_box("stack_p" + str(i), next_idx, self.N_SEATS),
                    # .................. current_bet
                    get_new_box("curr_bet_p" + str(i), next_idx, self.N_SEATS),
                    # ............................ is_allin
                    get_discrete(1, "is_allin_p" + str(i), next_idx),
                ]

                # add to parts_dict for possible slicing for agents
                obs_parts_idxs_dict["players"][i] += list(
                    range(_k, next_idx[0]))
            ...
            # ....................................................... side pots
            for i in range(self.N_SEATS):
                _table_space.append(get_new_box(
                    "side_pot_" + str(i), next_idx, 1))
            ...
            for i in range(self.N_SEATS):
                ...
                _player_space += [
                    ...
                    get_new_box("curr_bet_p" + str(i), next_idx, self.N_SEATS),
                    # ............. folded_this_epis
                    get_discrete(1, "has_folded_this_episode_p" + \
                                 str(i), next_idx),
                    # ............................ is_allin
                    get_discrete(1, "is_allin_p" + str(i), next_idx),
                ]
    ...
    def _init_from_args(self, env_args, is_evaluating):
        ...
        try:
            ...
            self.MAX_CHIPS = a.n_seats * \
                (self.DEFAULT_STACK_SIZE + a.stack_randomization_range[1]) + 1

        self.STACK_RANDOMIZATION_RANGE = a.stack_randomization_range

        if a.scale_rewards:
            try:
                self.REWARD_SCALAR = float(
                    sum(a.starting_stack_sizes_list)) / float(a.n_seats) / 5
            except TypeError:  # stack size set to None -> Default
        ...
        self.seats = [
            PokerPlayer(seat_id=i,
                        ...
                        starting_stack=(a.starting_stack_sizes_list[i]
    ...
    def _deal_turn(self):
        self.board[self.N_FLOP_CARDS:self.N_FLOP_CARDS +
                   self.N_TURN_CARDS] = self.deck.draw(self.N_TURN_CARDS)

    def _deal_river(self):
        d = self.N_FLOP_CARDS + self.N_TURN_CARDS
        self.board[d:d +
                   self.N_RIVER_CARDS] = self.deck.draw(self.N_RIVER_CARDS)
    ...
    def _payout_pots(self):
        ...
        if self.N_SEATS == 2:
            ...
            for e in pot_and_pot_ranks:
                ...
                rank = e[1]
                eligible_players = [
                    p for p in self.seats if p.side_pot_rank >= rank and not p.folded_this_episode]
                ...
                if num_eligible > 0:
                    ...
                    winner_list = self._get_winner_list(
                        players_to_consider=eligible_players)  # PokerPlayer objects
    ...
    def _put_current_bets_into_main_pot_and_side_pots(self):
        ...
        if self.N_SEATS == 2:
            ...
            dif_p0_to_p1 = self.seats[0].current_bet - \
                self.seats[1].current_bet
            ...
            dif = _players_sorted_by_bet_in_front[0].current_bet - \
            ...
            players_not_folded = [
                p for p in self.seats if not p.folded_this_episode]

            # The smallest bet of someone who did not fold.
            # If no one is allin, all nonfold bets are equal and put into main pot
            main_pot_max_amount = min(
                [p.current_bet for p in players_not_folded])
    ...
    def _next_round(self):
        ...
        # highly dependant on PokerEnv.""ROUND_NAME"" being sequential ints!
        self.current_round += 1
    ...
    def _step(self, processed_action):
        ...
        if processed_action[0] == Poker.CHECK_CALL:
            ...
            if processed_action[1] < self._get_current_total_min_raise():
                ...
                self.capped_raise.player_that_cant_reopen = self.last_raiser
        ...
        self.last_action = [processed_action[0],
                            processed_action[1], self.current_player.seat_id]
        ...
        if self._should_continue_in_this_round(all_non_all_in_and_non_fold_p=all_non_all_in_and_non_fold_p,
            ...
            if self.RETURN_PRE_TRANSITION_STATE_IN_INFO:
                info = {"chance_acts": False,
                        "state_dict_before_money_move": None}
        ...
        # only one not folded, so pay all pots to him.
            ...
            raise RuntimeError(
                "There seems to be an edge-case not built into this")
    ...
    def _get_current_total_min_raise(self):
        ...
        if self.N_SEATS == 2:
            _sorted_ascending = sorted(
                [p.current_bet for p in self.seats])  # 0 is small, 1 is big
            delta = max(_sorted_ascending[1] -
                        _sorted_ascending[0], self.BIG_BLIND)
            ...
            for i in range(1, self.N_SEATS):
                ...
                delta_between_last_and_before_last = _largest_bet - \
                    current_bets_sorted_descending[i]
    ...
    def _get_fixed_action(self, action):
        ...
        if _action_idx == Poker.FOLD:
            ...
            raise RuntimeError(
                'invalid action ({}) must be fold (0), call (1), or raise (2) '.format(_action_idx))

    def _process_check_call(self, total_to_call):
        delta_to_call = min(
            total_to_call - self.current_player.current_bet, self.current_player.stack)
        total_bet_to_be_placed = int(
            delta_to_call + self.current_player.current_bet)
    ...
    def _get_table_state(self, normalization_sum):
        if (self.N_SEATS == 2) and self._USE_SIMPLE_HU_OBS:
            community_state = [
                ...
                self.last_action[1] /
                normalization_sum if self.last_action[0] is not None else 0,
            ]
            ...
            if self.N_SEATS > 2:
                community_state += [sp /
                                    normalization_sum for sp in self.side_pots]
            else:
    ...
    def reset(self, deck_state_dict=None):
        ...
        if self.IS_FIXED_LIMIT_GAME:
            if self.BIG_BLIND > 0:
                # big blind counts, but in ante-only games like LEDUC it doesn't count
                self.n_raises_this_round = 1
            else:
                self.n_raises_this_round = 0
        ...
        # start new game
        self._post_antes()
        # antes don't count to current bet
        self._put_current_bets_into_main_pot_and_side_pots()
    ...
    def step_raise_pot_frac(self, pot_frac):
        ...
        """
        processed_action = (2, self.get_fraction_of_pot_raise(
    ...
    def load_state_dict(self, env_state_dict, blank_private_info=False):
        ...
        self.last_action = copy.deepcopy(
            env_state_dict[EnvDictIdxs.last_action])
        ...
        for p in self.seats:
            ...
            p.has_acted_this_round = env_state_dict[EnvDictIdxs.seats][
                p.seat_id][PlayerDictIdxs.has_acted_this_round]
    ...
    def get_current_obs(self, is_terminal):
        ...
        normalization_sum = float(
            sum([s.starting_stack_this_episode for s in self.seats])) / self.N_SEATS
        return np.array(self._get_table_state(normalization_sum=normalization_sum)
                        ...
                        + self._get_board_state(), dtype=np.float32)
    ...
    def get_fraction_of_pot_raise(self, fraction, player_that_bets):
        ...
        """
        _player = player_that_bets if isinstance(
            player_that_bets, PokerPlayer) else self.seats[player_that_bets]
        to_call = self._get_biggest_bet_out_there_aka_total_to_call() - \
            _player.current_bet
        pot_after_call = self.main_pot + \
            sum(self.side_pots) + \
            sum([p.current_bet for p in self.seats]) + to_call
    ...
    def human_api_ask_action(self):
        ...
        if action_idx == Poker.BET_RAISE:
            raise_size = int(
                input("To how many chips total do you want to raise?"))
        else:
            raise_size = 0
    ...
    def set_args(self, env_args):
        self._init_from_args(
            env_args=env_args, is_evaluating=self.IS_EVALUATING)
